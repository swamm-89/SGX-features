...
-- == 🎨 AESTHETIC THEME (Dark Inside, Purple Outside) ==
local Theme = {
	Background = Color3.fromRGB(12, 12, 14),   -- Pure Sleek Dark (No purple tint)
	Header = Color3.fromRGB(18, 18, 22),       -- Slightly lighter dark for header
	Surface = Color3.fromRGB(24, 24, 28),      -- Dark grey for buttons/elements
	Accent = Color3.fromRGB(180, 0, 255),      -- Pure Neon Purple 💜
	Text = Color3.fromRGB(255, 255, 255),      -- Pure White
	SubText = Color3.fromRGB(170, 170, 170),   -- Clean Grey
	ToggleOff = Color3.fromRGB(40, 40, 45),    -- Grey for off switches
	CloseBtn = Color3.fromRGB(255, 60, 60),    -- Bright Red
	MinBtn = Color3.fromRGB(255, 255, 255)     -- White
}

-- ScreenGui Setup
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "Saksham On Top"
ScreenGui.Parent = CoreGui
ScreenGui.ResetOnSpawn = false
ScreenGui.IgnoreGuiInset = true

-- ==========================================
-- 🎬 1. GUARANTEED INTRO ANIMATION
-- ==========================================
...
IntroText.Text = "Saksham On Top"
IntroText.Font = Enum.Font.GothamBlack
IntroText.TextSize = 20 -- Starts small
...
IntroGlow.Color = Theme.Accent; IntroGlow.Thickness = 0; IntroGlow.Transparency = 1
...
MainStroke.Transparency = 0.1 -- High visibility

-- ==========================================
-- ✨ 3. BETTER VISIBLE ANIMATED BACKGROUND
-- ==========================================
...
task.spawn(function()
	while task.wait(0.12) do
		...
		local size = math.random(3, 6) -- Bigger particles
		sprinkle.Size = UDim2.new(0, size, 0, size)
		
		local startX = math.random(0, 100) / 100
		sprinkle.Position = UDim2.new(startX, 0, 1.1, 0)
		sprinkle.BackgroundColor3 = Theme.Accent
		sprinkle.BackgroundTransparency = math.random(20, 60) / 100 -- More visible now
...
-- ==========================================
-- 🛠️ 4. HEADER & PERFECT BUTTON POSITIONS
-- ==========================================
...
TitleLabel.Text = "Follow Player by Saksham"
...
-- 🔴 Close Button (Shifted left so it doesn't hide behind curves)
...
CloseBtn.MouseEnter:Connect(function() TweenService:Create(CloseBtn, TweenInfo.new(0.2), {TextSize = 16}):Play() end)
CloseBtn.MouseLeave:Connect(function() TweenService:Create(CloseBtn, TweenInfo.new(0.2), {TextSize = 14}):Play() end)
...
MinBtn.MouseEnter:Connect(function() TweenService:Create(MinBtn, TweenInfo.new(0.2), {TextColor3 = Theme.Accent}):Play() end)
MinBtn.MouseLeave:Connect(function() TweenService:Create(MinBtn, TweenInfo.new(0.2), {TextColor3 = Theme.MinBtn}):Play() end)
...
PlayerScroll.BackgroundColor3 = Theme.Background
PlayerScroll.BackgroundTransparency = 1 -- Hide scroll background to look cleaner
...
-- 🎯 CURRENT TARGET INDICATOR (Clean look)
...
local function RefreshPlayers()
	...
	for _, p in pairs(Players:GetPlayers()) do
		if p ~= LocalPlayer then
			...
			-- Hover Effects (Clean Grey to Lighter Grey)
			Btn.MouseEnter:Connect(function() if TargetPlayer ~= p then TweenService:Create(Btn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(40, 40, 45)}):Play() end end)
			Btn.MouseLeave:Connect(function() if TargetPlayer ~= p then TweenService:Create(Btn, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Surface}):Play() end end)
			
			Btn.MouseButton1Click:Connect(function()
				TargetPlayer = p
				CurrentTargetLabel.Text = "🎯 TARGET: " .. string.upper(p.Name)
				CurrentTargetLabel.TextColor3 = Theme.Accent -- Turns purple when selected
				
				for _, b in pairs(PlayerScroll:GetChildren()) do
					if b:IsA("TextButton") then TweenService:Create(b, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Surface, TextColor3 = Theme.Text}):Play() end
				end
				-- Selected state is Pure Purple
...
local function CreateToggle(text, callback)
	...
	Btn.MouseButton1Click:Connect(function()
		...
		callback(isOn)
		if isOn then
			TweenService:Create(Indicator, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {BackgroundColor3 = Theme.Accent}):Play()
			TweenService:Create(Knob, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Position = UDim2.new(1, -16, 0.5, -7), BackgroundColor3 = Color3.fromRGB(255, 255, 255)}):Play()
		else
			TweenService:Create(Indicator, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {BackgroundColor3 = Theme.ToggleOff}):Play()
			TweenService:Create(Knob, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Position = UDim2.new(0, 2, 0.5, -7), BackgroundColor3 = Color3.fromRGB(200, 200, 200)}):Play()
		end
	end)
end
...
-- ==========================================
-- 🎚️ 6. SLIDER (FIXED FOR MOBILE/TOUCH)
-- ==========================================
...
local function UpdateSlider(input)
	...
	TweenService:Create(SliderFill, TweenInfo.new(0.05, Enum.EasingStyle.Sine), {Size = UDim2.new(p, 0, 1, 0)}):Play()
...
RunService.RenderStepped:Connect(function(dt)
	if FollowEnabled and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("HumanoidRootPart") then
		if Character and HRP then
			...
			if FaceTarget then goal = CFrame.new(HRP.Position, tHRP.Position) * CFrame.new(0, 0, FollowDistance) end
